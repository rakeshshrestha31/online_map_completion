...
if __name__ == '__main__':
    ...
    with open(args.cached_avg_data, 'r') as f:
        all_avg_data_dict = json.load(f, object_pairs_hook=OrderedDict)
    common_floorplan = list(next(iter(all_avg_data_dict.values())).keys())
    
    outputs = []
    for floorplan in common_floorplan:
        for x_label in [SIM_TIME_LABEL, TRAJECTORY_LABEL]:
            label_data_tuples = list(all_avg_data_dict.items()) # sorted(list(all_avg_data_dict.items()))
