...
from kth.FloorPlanEntity import *
import os


class GraphFileOperations(object):
    @classmethod
    def updateExtent(cls, graph):
        maxx = float("-inf")
        maxy = float("-inf")
        minx = float("inf")
        miny = float("inf")

        for spaceName in graph.nodes():
            space = graph.nodes[spaceName]["attr"]
            minx = min(minx, space.minx)
            miny = min(miny, space.miny)
            maxx = max(maxx, space.maxx)
            maxy = max(maxy, space.maxy)

        graph.graph["attr"].minx = minx
        graph.graph["attr"].miny = miny
        graph.graph["attr"].maxx = maxx
        graph.graph["attr"].maxy = maxy

        graph.graph["attr"].centroid.x = (minx + maxx) / 2
        graph.graph["attr"].centroid.y = (miny + maxy) / 2
                        ...
                        if i.tag == "portal":
                            if "target" in i.attrib:
                                ...
                                if (sourceSpaceName, targetSpaceName) not in outGraph.edges and \
                                        sourceSpaceName != targetSpaceName and \
                                        targetSpaceName in outGraph.nodes:  # ignore wrong targetName
                                    outGraph.add_edge(sourceSpaceName, targetSpaceName)

        outGraph.graph["attr"] = currentGraphProperty
        cls.updateExtent(outGraph)
        updated_property = outGraph.graph["attr"]

        return outGraph, updated_property
